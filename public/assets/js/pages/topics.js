$(document).ready(function () {
    ...
    $(document).on('change', 'select#category_id', function () {
        let id = $(this).val()
        $('select#sub_category_id option').attr('disabled', 'disabled');
        $(`select#sub_category_id`).val('');
        if (id) {
            $(`select#sub_category_id option[data-parent=${id}]`).removeAttr('disabled');
        }
        subcatSelect2();
    });
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    $('#add-modal').modal('show');
                    $('#add-form').find('#category_id').val(result.data.category_id).trigger('change');
                    $('#add-form').find('#sub_category_id').val(result.data.sub_category_id);
                    $('#add-form').find('#topic').val(result.data.topic);

                    setTimeout(function () {
                        if ($('#add-form').length > 0) {
                            subcatSelect2();
                        }
                        $('#add-form').find('#sub_category_id').val(result.data.sub_category_id).change();
                    }, 200);
