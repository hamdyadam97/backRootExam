$(document).ready(function () {
            ...
            },
            {
                ...
                render: function (_, _, full) {
                    ...
                    if (contactId) {
                        ...
                        actions += ' <a href="javascript:void(0)" data-id="' + contactId + '" class="waves-effect waves-light pe-2 copy-exam" title="copy exam"><i class="bx bx-copy bx-sm"></i></a>';
    ...
    function getSubSubCategory(subcategoryId,sub_sub_cat_id="") {
        if (subcategoryId) {
            $.ajax({
                url: sub_subcatUrl +'?id='+ subcategoryId,
                type: 'GET',
                dataType: 'json',
                success: function(response) {
                    $('#sub_sub_cat_id').empty();
                    $('#sub_sub_cat_id').append('<option value="">Select sub-subcategory</option>');
                    if (response.status == true) {
                        $.each(response.data, function(key, value) {
                            if(sub_sub_cat_id==key){
                                $('#sub_sub_cat_id').append('<option selected value="' + key + '">' + value + '</option>');
                            }else{
                                $('#sub_sub_cat_id').append('<option value="' + key + '">' + value + '</option>');
                            }
                        });
                    }
                }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.status) {
                    ...
                    getSubSubCategory(result.data.sub_cat_id,result.data.sub_sub_cat_id);
                    setTimeout( function(){
    ...
    $('body').on('click', '.copy-exam', function (e) {
        var id = $(this).attr('data-id');
        Swal.fire({
            title: 'Are you sure to copy exam',
            icon: 'info',
            showCancelButton: true,
